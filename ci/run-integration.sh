...
#   VGI_DOCGEN_WORKER   worker LOCATION the .test files ATTACH (a stdio command
#                       such as `.venv/bin/vgi-docgen`, or an http:// URL)
# Optional:
#   STAGE                scratch dir for the preprocessed test tree (default: mktemp)
set -euo pipefail

: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_DOCGEN_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
# These .test files name DOCX template fixtures by a path relative to the runner
# cwd (e.g. `test/sql/data/invoice.docx`); the worker reads them from its own
# cwd, which is $STAGE. Stage the committed fixtures alongside the preprocessed
# tests so the worker resolves them.
...
echo "Running suite (worker: $VGI_DOCGEN_WORKER) ..."
