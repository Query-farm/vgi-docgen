...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-docgen
...
# ATTACH (`${VGI_DOCGEN_WORKER}`):
#
#   subprocess : a bare stdio command (`.venv/bin/vgi-docgen`) — the extension
...
#   VGI_DOCGEN_WORKER   the stdio command that runs the worker (the installed
#                       `vgi-docgen` console script in the project venv). Used
...
: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_DOCGEN_WORKER:?worker LOCATION (stdio command running the worker)}"
...
# VGI_DOCGEN_WORKER with a URL.
WORKER_CMD="$VGI_DOCGEN_WORKER"
...
# The .test files name DOCX template fixtures by a path relative to the runner
# cwd (e.g. `test/sql/data/invoice.docx`); the worker reads them from its own
# cwd. The runner cd's into the stage dir, and for http/unix we boot the worker
# with cwd = the stage dir too, so the fixtures must live under $STAGE. Copy the
# committed data dir in.
if [ -d "$REPO/test/sql/data" ]; then
  cp -R "$REPO/test/sql/data" "$STAGE/test/sql/data"
fi

# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_DOCGEN_WORKER (the LOCATION) and, for the
...
case "$TRANSPORT" in
  subprocess)
    echo "Transport: subprocess/stdio — VGI_DOCGEN_WORKER=$VGI_DOCGEN_WORKER"
    ;;
  ...
  http)
    ...
    # cwd = STAGE so the worker resolves the staged relative-path fixtures
    # (test/sql/data/*.docx) exactly like the runner does.
    PORT_FILE="$(mktemp -u "${TMPDIR:-/tmp}/dg-port.XXXXXX")"
    LOG_FILE="${TMPDIR:-/tmp}/dg-http-server.log"
    ...
    export VGI_DOCGEN_WORKER="http://127.0.0.1:$PORT"
    echo "HTTP worker ready on $VGI_DOCGEN_WORKER (pid $SERVER_PID)"
    ;;
  ...
  unix)
    ...
    SOCK="${TMPDIR:-/tmp}/dg-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/dg-unix-server.log"
    ...
    export VGI_DOCGEN_WORKER="unix://$SOCK"
    echo "unix worker ready on $VGI_DOCGEN_WORKER (pid $SERVER_PID)"
    ;;
esac
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_DOCGEN_WORKER) ..."
