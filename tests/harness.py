"""In-process driver for the docgen buffering (Sink+Source) merge function.

Runs ``docgen_merge`` through its real bind -> init -> process(sink) -> combine
-> finalize lifecycle without spawning a worker process, so unit tests stay fast
and debuggable while still exercising argument parsing, the storage round-trip,
and the output schema. Adapted from the vgi-statsmodels harness.
"""
...
def run_buffering(
    ...
    *,
    ...
    """Drive a buffering function over a whole input ``table``.

    Args:
        func_cls: The ``TableBufferingFunction`` subclass to run.
        table: The input relation (the ``(SELECT ...)`` data) as an Arrow table.
        named: Named args (e.g. ``{"template": "invoice.docx"}``).

    Returns:
        The emitted result as a single Arrow table.
    """
